...
async function loadWorkDetail() {
  ...
  try {
    ...
    if (work) {
      ...
      if (mainWindow) {
        mainWindow.innerHTML = '<p style="padding: 40px; color: #888;">作品が見つかりませんでした。<a href="/">一覧に戻る</a></p>';
      }
    }
  } catch (error) {
...
function renderRecommendations(currentWork, allWorks, knownMaterials) {
  ...
  if (savedRecommend) {
    ...
    savedWorks.forEach(work => {
      const html = `
        <div class="work-item">
          <a href="/work?p=${work.en_name}" class="work-item-link">
...
//astro移植時の変更点
//fetchでお見込むデータの名前の先頭に/を追加
//リンクの表記パスをastro用に変更
//最後のdocument.add~の記述を変更